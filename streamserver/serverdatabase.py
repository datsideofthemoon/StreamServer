...
class ServerDatabase:
	def insert_many(self, table, fields=(), values=[]):
		with app.app_context():
			...
			cur = db.cursor()
			for row in values:
				query = u'INSERT INTO %s (%s) VALUES (%s)' % (table,', '.join(fields),', '.join(['?'] * len(row)))
	...
	def get_range(self, start, count):
		return self.query_db('select * FROM mediafiles WHERE id >= (%s) AND id <= (%s)' % (start, start+count-1),one=False)
		
	def get_by_idx(self,idx):
		max=self.query_db('select Count(*) from mediafiles')[0]
		if idx>0 and idx <= max and max >= 1:
			return self.query_db('select 1 FROM mediafiles WHERE id = (%s)' % (idx),one=True)[0]
		else:
			return None
			...
			if row['foreign_keys']==0:
				...
				for rw in rws:
					if rw['foreign_keys']==0:
						raise Exception("Database PRAGMA FOREIGN_KEYS CANNOT BE TURNED ON! Recompile sqlite3 with FOREIGN_KEYS on.")
					else:
